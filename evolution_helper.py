...
def build_base_nn_eval_func(x_train, y_train, x_test, y_test, max_num_layers=None,
    ...
    transcription = transcribe_base_rnn if recurrent else transcribe_base_cnn if convolutional else transcribe_base
    def eval_func(sequence, ret=False, x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test, k_folds=k_folds, verbose=verbosity, metric=metric, epochs=epochs, gene_len=False, transcription=transcription, recurrrent=recurrent, convolutional=convolutional):
        if gene_len:
            return transcription(sequence, max_num_layers, max_num_neurons, activations=activations, optimizers=optimizers, dropouts=dropouts, scalers=scalers, gene_len=True)
        if recurrent:
        ...
        metric = metric if metric else accuracy_score
